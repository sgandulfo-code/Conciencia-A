...
export function SplitScreen({ data }: SplitScreenProps) {
  return (
    <div className="space-y-6">
      {/* Warning for minors */}
      {data.id === 'adolescentes' && (
        <motion.div 
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          className="bg-red-100 border-l-4 border-red-600 p-5 rounded-r-2xl shadow-sm flex items-start gap-4"
        >
          <AlertTriangle className="text-red-600 shrink-0 mt-0.5" size={28} />
          <div>
            <h4 className="text-red-900 font-bold text-lg">Aviso Importante: Menores de Edad</h4>
            <p className="text-red-800 mt-1 leading-relaxed">
              El consumo de alcohol en menores de 18 años es ilegal y altamente perjudicial. El cerebro adolescente es especialmente vulnerable a los efectos tóxicos del alcohol, lo que puede causar daños irreversibles en el desarrollo cognitivo y emocional. La única recomendación segura es <strong>cero alcohol</strong>.
            </p>
          </div>
        </motion.div>
      )}
